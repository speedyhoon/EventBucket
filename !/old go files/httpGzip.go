...
func gzipperFiles(h http.Handler, w http.ResponseWriter, r *http.Request) {
	//Return a gzip compressed response if appropriate
	//	fmt.Println("\n\nFiles")
	//	fmt.Printf("%#v", r.URL.Path)
	//	if r.URL.Path == "/j/sizzle.min.js" {
	//		temp := 7
	//	}
	//	fmt.Println("-------------")
	//	fmt.Println(w)
	//	fmt.Println("files", r.Referer())
	//	if strings.Contains(r.Header.Get("Accept-Encoding"), "gzip") {
	//		w.Header().Set("Content-Encoding", "gzip")
	//		gz := gzip.NewWriter(w)
	//		defer gz.Close()
