...
//	filepath.Walk(rootDir + "sass", walkPath)
	...
	filepath.Walk(rootDir + "css", walkPath)
...
func walkPath(path string, f os.FileInfo, err error) error {
	...
	if !f.IsDir() {
		...
		switch currentDir{
		case "golang":
			if f.Name() != "unusedCode" {
				err = ioutil.WriteFile(copyToDir+f.Name(), source, 0777)
			}
		case "js":
			err = ioutil.WriteFile(copyToDir+"j/"+f.Name(), source, 0777)
			gzipFile(source, copyToDir+"j/"+f.Name())
		case "css":
			err = ioutil.WriteFile(copyToDir+"c/"+f.Name(), source, 0777)
			gzipFile(source, copyToDir+"c/"+f.Name())
		case "html":
			...
			gzipFile(source, copyToDir+"/h/"+strings.Replace(f.Name(), ".html", ".htm", -1))
		}
		if err != nil{
...
func gzipFile(source []byte, fileName string){
	var b bytes.Buffer
	w := gzip.NewWriter(&b)
	w.Write(source)
	w.Close()
	if b.Len() < len(source) {
		fmt.Println(fileName, "new Length: ", len(source) - b.Len())
		err := ioutil.WriteFile(fileName + ".gz", b.Bytes(), 0666)
		if err != nil{
			fmt.Println(err)
		}
	}
