...
// A list of form elements & their available attributes that can be used. Many unused attributes have been omitted.
...
//	}
	...
	for i, input := range form.field {
		...
		if input.class != "" {
			elementClass = append(elementClass, input.class)
		}
		if input.help != "" && input.class != "il" {
		...
		if input.help != "" {
			...
			//			elementHelp = "<img src=/p/help2 alt=? title='" + input.help + "'>"
			elementHelp = "<c title='" + input.help + "'>❔<div>" + input.help + "</div></c>" //U+2754: WHITE QUESTION MARK ORNAMENT
		}
		if input.label != "" {
			...
			if input.error != "" {
				...
				//				element = "<label class=" + addQuotes(strings.Join(labelClasses, " ")) + ">" + input.label + element + "<p>" + input.error + "</p></label>"

				element = "<label class=error>" + input.label + elementHelp + element + "<p>" + input.error + "</p></label>"
			} else {
				//				class := addQuotes(strings.Join(labelClasses, " "))
				//				if class != "" {
				//					class = " class=" + class
				//				}
				//				element = "<label" + class + ">" + input.label + element + "</label>"
				element = "<label>" + input.label + elementHelp + element + "</label>"
			}
		}
...
func drawOptions(input field) string {
