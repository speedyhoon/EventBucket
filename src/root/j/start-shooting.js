(function(){
	...
	function getValue(thisCell, attribute){
		var temp = thisCell.getAttribute('data-' + attribute);
		return (temp === null || temp === undefined || isNaN(temp)) ? 0 : parseInt(temp, 10);
	}

	function recalculateTotal(value){
		var type = getShootersClass(),
			index = classes[type].validShots.indexOf(value),
			newer = parseInt(classes[type].validScore[index], 10),
			newerC = parseInt(classes[type].validCenta[index], 10),
			total,
			centers;
//			old,
//			oldC;
...
//			total = getValue(currentRow, 'total');
//			centers = getValue(currentRow, 'centers');
//			old = getValue(currentCell, 'value');
//			oldC = getValue(currentCell, 'center');
	...
	function ajax(id){
		...
		j.open('POST', '/p/ctrl.php', true);// + 'scoreSave=' + classes.eventId + '~' + Id + '~' + shots, true);
	...
	function changeValue(value){
		if(currentCell){
			if(currentCell.innerHTML !== value){//prevents recalculating the score if it is the same value
				...
				/*var selected = document.getElementById('selectSighters').selectedIndex;
				var currentNth = getCurrentNth();
				if(selected > 0 && currentNth < getNoOfSighters()){
					//var tempCell = currentRow.getElementsByTagName('td')[currentNth+selected];
					currentCell = currentRow.getElementsByTagName('td')[currentNth+selected];
					if(currentNth >= selected){
						//recalculateTotal(tempCell,value);
						recalculateTotal(value);
					}
					//highlightCell(currentCell.nextSibling);
					//return;
				}*/
	...
	function buttonsAddEvents(){
		var n = 0, buttons = document.getElementsByTagName('button'), max = buttons.length, buttonOnClickEvent = function(button_value){
				return function(){
					changeValue(button_value);
				};
			};
		for(n; n < max; n++){
	...
	function changeSighters(){//when changing the value of the select box "sighters"
		var selected = document.getElementById('selectSighters').selectedIndex, tds, iteration, sighters, seletedCell, i;
		if(currentRow && selected > 0){
			tds = currentRow.getElementsByTagName('td');
			iteration = 0;
			sighters = getNoOfSighters();
			seletedCell = currentCell;
			for(i = sighters - selected; i < sighters; i++){
				//recalculateTotal(tds[sighters+iteration++],tds[i].innerHTML);//increment iteration AFTER this line
				currentCell = tds[sighters + iteration++];
				recalculateTotal(tds[i].innerHTML);//increment iteration AFTER this line
			}
			currentCell = seletedCell;
	...
	function runOnLoad(){
		var n = 0, shooters = document.getElementsByClassName('name'), shooterQuantity = shooters.length, shooterNameOnclick = function(tr_element){
				return function(){
					highlightRow(tr_element);
	...
	runOnLoad();//178,161,166,168,238,260
	//6328,5685
}());