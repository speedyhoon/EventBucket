...
func generator(w http.ResponseWriter, fillin string, data map[string]interface{}) {
	my_html := template.New("my_template").Funcs(template.FuncMap{
		...
		"JSStr": func(x string) template.JSStr {
...
func event_menu(event_id string, event_ranges map[string]Range, page_url string, isPrizeMeet bool) string {
	menu := "<ul>"
	selected := ""
			...
			if (isPrizeMeet && menu_item.Link != URL_totalScores) || !isPrizeMeet{
				...
				for range_id, range_item := range event_ranges {
					if len(range_item.Aggregate) == 0 && !range_item.Hidden {
						menu += fmt.Sprintf("<li><a href=%v%v/%v>%v</a></li>", addQuotes(menu_item.Link), event_id, range_id, range_item.Name)
					}
				}
				menu += "</ul></li>"
			}
...
func scoreboard_menu(event_id string, event_ranges map[string]Range, page_url string, isPrizeMeet bool) string {
	menu := "<ul>"
	selected := ""
	for _, menu_item := range EventMenuItems {
		if menu_item.Link == page_url{
