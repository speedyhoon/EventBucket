(function(){
	'use strict';
	var currentCell = null, currentRow = null, j, previous_grade/*, classes = {
		eventId:1,
		currentType:null,
		target:{
			sighters:2,
...
//		j.send('scoreSave=' + classes.eventId + '~' + id + '~' + shots);
		j.send('event_id=' + classes.eventId + '&range_id=' + classes.rangeId + '&shooter_id=' + id + '&shots=' + shots);
	...
	function generateButtons(){
		...
		if(classes.currentType !== type){
			...
			if(type && classes[type] && classes[type].buttons){
	...
	function moveHeader(){
		...
		generateButtons();
		if(previous_grade !== getShootersClass()){
			var heading_cells = document.getElementById('h').getElementsByTagName("td"), max = heading_cells.length;
			for(var i = 0; i < max; i++){
				heading_cells[0].remove();
			}
			var heading = document.getElementById('h');
			var th_tags = heading.getElementsByTagName("th");
			var total_tag = th_tags[th_tags.length - 1];
			var new_tds = document.getElementById(getShootersClass() + "_heading_cells").cloneNode(true).getElementsByTagName("td");
			var i = 0, max = new_tds.length;
			for(i = 0; i < max; i++){
				heading.insertBefore(new_tds[0], total_tag);
			}
		}
	}
