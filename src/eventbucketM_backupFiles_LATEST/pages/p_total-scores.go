...
func totalScores(w http.ResponseWriter, r *http.Request) {
	page_url := "/totalScores/"
	url := fmt.Sprintf("%v", r.URL)
	templator("admin", totalScores_HTML(), totalScores_Data(url[len(page_url):]), w)
}
func totalScores_Data(url string) map[string]interface{} {
...
func totalScores_HTML() string {
	return loadHTM("total-scores")
}
