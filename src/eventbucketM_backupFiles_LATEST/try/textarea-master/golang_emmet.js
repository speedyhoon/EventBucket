...
//			return result;
	...
	function parseAbbreviation(abbr) {
		...
		var context = root.addChild(), ch, tempCh;
		...
		while (!stream.eol() && --loopProtector > 0) {
			...
			if(false){
			/*if(ch == "\n"){
				tempCh = stream.string[stream.pos+1];
				if(tempCh == "\t"){
					tempCh = ch + tempCh + stream.string[stream.pos+2];
				}*/
			}else{
				tempCh = ch;
			}
...
emmet.define('utils', function(require, _) {
	...
	return {
		...
		splitByLines: function(text, removeEmpty) {
			...
			//console.log(removeEmpty, lines, text);
			return lines;
		},
...
//			console.log(textLength>=3, text[0]==='{', text[textLength-1]==='}', text[1]!=='{', text[textLength-2]!=='}');
			if(textLength>=3 && text[0]==='{' && text[textLength-1]==='}' && text[1]!=='{' && text[textLength-2]!=='}'){
				return '{' + text + '}';
...
emmet.define('editor', function(require, _) {
	...
	return {
		...
		replaceContent: function(value, start, end, noIndent) {
			...
			document.querySelector("#iframe").src = 'data:text/html;charset=utf-8,'+utils.replaceSubstring(content, value, start, end);
		},
...
emmet.define('textarea', function(require, _) {
	 ...
	 */
	var previousTextAreaValue = document.querySelector("textarea").value
	function runAction(){
		require('actions').run('expand_abbreviation', require('editor'));
	}

	var options = {syntax: "html", use_tab: false, pretty_break: false};

	document.querySelector("textarea").onkeyup = (function(runAction, previousTextAreaValue){
		return function(){
			if (this.value !== previousTextAreaValue){
				runAction();
				previousTextAreaValue = this.value;
			}
		}
	})(runAction, previousTextAreaValue);
	...
	return {
		...
		getOption: function (name) {
			return {syntax: "html", use_tab: false, pretty_break: false}[name];
		}
	};
});