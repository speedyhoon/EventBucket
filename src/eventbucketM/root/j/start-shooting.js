(function(){
	'use strict';
	var currentCell = null, currentRow = null, j, previousGrade,
		currentType = null,
		classes = [
			{//target
				Buttons:'012345VX',
	...
	if (window.localStorage){
		if (localStorage.length) {
			...
			//make a request to get these data!
	...
	function getNoOfSighters(){
		return classes[getShootersClass()].SightersQty;
	}
	...
	function recalculateTotal(value){
		...
		if(getCurrentNth() >= getNoOfSighters()){
			...
			if(centers){
				currentRow.lastChild.innerHTML = total + '.' + centers;
			}else{
				currentRow.lastChild.innerHTML = total
			}
	...
	function getAjax(){
		var cells = currentRow.getElementsByTagName('td'),
			max = cells.length,
			shootersClass = classes[getShootersClass()],
	...
	function ajax(id){
		var shots = encodeURI(getAjax()).replace(/#/gi, '%23');	//hashes are converted after encodeURI to stop % being converted twice
		j.open('POST', window.location.origin+'/updateShotScores', true);// + 'scoreSave=' + classes.eventId + '~' + Id + '~' + shots, true);
		j.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
//		j.send('scoreSave=' + classes.eventId + '~' + id + '~' + shots);
		j.send('event_id=' + eventId + '&range_id=' + rangeId + '&shooter_id=' + id + '&shots=' + shots);
		j.onreadystatechange = function(){
			if(j.status === 200 && j.readyState === 4){
	...
	function generateButtons(){
		var type = getShootersClass();
		if(type >= 0 && currentType !== type && classes[type] && classes[type].Buttons){
			currentType = type;
			var n,
				buttonLength = classes[type].Buttons.length,
				fragment = document.createDocumentFragment(),
				buttonOnClickEvent = function(buttonValue){
					return function(){
			...
			for(n = 0; n < buttonLength; n++){
				...
				button.textContent = classes[type].Buttons[n];
				button.onclick = buttonOnClickEvent(classes[type].Buttons[n]);
			}
			document.getElementById('bu').appendChild(fragment);
		}
	}
	...
	function moveHeader(){
		var tbody = currentRow.parentNode,
			...
			thTags = heading.getElementsByTagName('th'),
			totalTag = thTags[thTags.length - 1],
			shootersClass = getShootersClass(),
			newTdsRow = document.getElementById('heading_cells'+shootersClass).cloneNode(true).getElementsByTagName('td');
		tbody.insertBefore(heading, currentRow);
		tbody.insertBefore(document.getElementById('x'), currentRow.nextSibling);//equivilent to insertAfter!
		generateButtons();
		if(previousGrade !== shootersClass){
			var headingCells = document.getElementById('h').getElementsByTagName('td'),
				max = headingCells.length,
				i;
			for(i = 0; i < max; i++){
				headingCells[0].remove();
			}
			for(i=0, max = newTdsRow.length; i < max; i++){
				heading.insertBefore(newTdsRow[0], totalTag);
			}
		}
	}
...
//			modifySelectBox();
	...
	function runOnLoad(){
		var
			n = 0,
			shooters = document.getElementsByClassName('name'),
			shooterQuantity = shooters.length, shooterNameOnclick = function(trElement){
				return function(){
