...
func range_report(w http.ResponseWriter, r *http.Request) {
	...
	total := func(c1, c2 *EventShooter) bool {
		return c1.Scores[fmt.Sprintf("%v",range_id)].Total > c2.Scores[fmt.Sprintf("%v",range_id)].Total
	}
	centa := func(c1, c2 *EventShooter) bool {
		return c1.Scores[fmt.Sprintf("%v",range_id)].Centers > c2.Scores[fmt.Sprintf("%v",range_id)].Centers
	}
	cb := func(c1, c2 *EventShooter) bool {
		return c1.Scores[fmt.Sprintf("%v",range_id)].CountBack1 > c2.Scores[fmt.Sprintf("%v",range_id)].CountBack1
	}
