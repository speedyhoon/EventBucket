...
function shooterBarcode($search) {
  ...
  if (eventID !== barcodeEventID) {
    ...
    if (
      confirm(
        `This barcode is for a different event. Do you want to go to event with id ${barcodeEventID}?`
      )
    ) {
  ...
  if (rangeID !== barcodeRangeID) {
    ...
    if (
      confirm(
        `This barcode is for a different range. Do you want to go to range with id ${barcodeRangeID}?`
      )
    ) {
      window.location.href = `/${pathName}/${barcodeEventID}/${barcodeRangeID}#${shooterID}`;
    }
